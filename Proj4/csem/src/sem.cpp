...
call(char *f, struct sem_rec *args)
{
  ...
  Func = (Function*) id->i_value;
  while (args != NULL) {
	arguments.push_back((Value*) args->s_value);
	args = args->s_link;
  }

  callInst = Builder.CreateCall(Func, makeArrayRef(arguments), "");
...
ccand(struct sem_rec *e1, void *m, struct sem_rec *e2)
{
	...
	rec = node((void *)NULL, (void *)NULL, 0, (struct sem_rec *)NULL, e2->s_true, merge(e1->s_false, e2->s_false));
	return rec;
...
{
	sem_rec *rec;
	rec = node((void *)NULL, (void *)NULL, 0, (struct sem_rec *)NULL, e->s_false, e->s_true);
	return rec;
...
ccor(struct sem_rec *e1, void *m, struct sem_rec *e2)
{
    ...
    rec = node((void *)NULL, (void *)NULL, 0, (struct sem_rec *)NULL, merge(e1->s_true, e2->s_true), e2->s_false);
	return rec;
...
indx(struct sem_rec *x, struct sem_rec *i)
{
	...
	Value *value = Builder.CreateGEP( (Value*)x->s_value, makeArrayRef(v));
...
labeldcl(const char *id)
{
	...
	//br_inst->setSuccessor(0, (BasicBlock *)NULL);
	return;
...
set(const char *op, struct sem_rec *x, struct sem_rec *y)
{
	...
	if(*op == '/' || *op == '+' || *op == '-' || *op == '*' || *op == '%'){
		load = Builder.CreateLoad((Value *)x_cast->s_value);
...
{
	struct sem_rec *rec;
	rec = s_node((void*) Builder.CreateGlobalStringPtr(parse_escape_chars(s)), T_STR);
	return rec;
